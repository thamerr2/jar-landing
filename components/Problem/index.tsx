...
const problemIcons = [
  ...
  <svg key="3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6">
  ...
  return (
    <section id="problem" className="py-24 bg-white">
      <div className="max-w-7xl mx-auto px-6">
        {/* Left-aligned statement — not centered, editorial */}
        <motion.p
          initial={{ y: 20 }}
          whileInView={{ y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.6 }}
          className="text-3xl md:text-4xl lg:text-[2.6rem] font-bold text-primary mb-16 leading-tight max-w-2xl"
        >
          {t('tagline')}
        </motion.p>

        {/* Pain points — divider layout, not cards */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
          {cardKeys.map((key, i) => (
            <motion.div
              ...
              className="border-t-2 border-accent/35 pt-6 pb-10 lg:px-7 first:lg:ps-0 last:lg:pe-0"
            >
              <div className="mb-4 text-accent">{problemIcons[i]}</div>
              <h3 className="font-bold text-primary mb-2 text-base">{t(`${key}_title`)}</h3>
